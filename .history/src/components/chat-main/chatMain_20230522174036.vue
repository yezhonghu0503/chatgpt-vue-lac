    ...
    <div class="flex flex-col w-[34vw] h-[100%] border-r-2 border-indigo-500">
      ...
      >
        <div class="w-[95%] h-[70vh] text-start p-4 pl-0 overflow-auto">
          ...
          >
            <div ref="htmltemp" class="" v-html="htmlString"></div>
            <img
...
function outputElement(element: any) {
  if (element.nodeType === Node.TEXT_NODE) {
    ...
    for (let i = 0; i < element.childNodes.length; i++) {
      outputElement(element.childNodes[i]);
    }
    document.write("</" + element.tagName + ">");
  }
}
const getOutput = () => {
  outputElement(htmltemp.value);
  // console.log(htmltemp.value);
  // htmlRendering(htmltemp.value);
  // const tempstr: any =
...
const htmlRendering = (html: any) => {
  if (html.children.length > 0) {
    Array.from(html.children).forEach((element: any) => {
  ...
  // console.log(html);
  if (html.children.length !== 0) {
    // const tempStr = html.innerText;
    // let i = 0;
    // html.innerText = "";
    // while (i > tempStr.length) {
    //   html.innerText += tempStr.charAt(0);
    //   syncSetInterval(500);
    //   i++;
    // }
    console.log(html);
